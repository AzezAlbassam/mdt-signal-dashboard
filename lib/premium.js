...
export function shortStrangle ({
  S0, S1, sigma, days, widthSigma = 1, r = 0, skew = 0, costPerLeg = 0,
}) {
  const { move, callStrike, putStrike } = strikesFor({ S0, sigma, days, widthSigma })
  const T = days / DAY_BASIS
  const callPremium = bsPrice({ S: S0, K: callStrike, T, sigma: sigma * (1 - skew), r, type: 'C' })
  const putPremium = bsPrice({ S: S0, K: putStrike, T, sigma: sigma * (1 + skew), r, type: 'P' })
...
export function ironCondor ({
  S0, S1, sigma, days, widthSigma = 1, wingPoints, wingSigma, r = 0, skew = 0, costPerLeg = 0,
}) {
  const short = shortStrangle({ S0, S1, sigma, days, widthSigma, r, skew })
  ...
  const T = days / DAY_BASIS
  const longCall = bsPrice({ S: S0, K: short.callStrike + wingPoints, T, sigma: sigma * (1 - skew), r, type: 'C' })
  const longPut = bsPrice({ S: S0, K: short.putStrike - wingPoints, T, sigma: sigma * (1 + skew), r, type: 'P' })
...
export function simulatePremium (sessions, {
  ...
  ivScale = 1, skew = 0, costPerLeg = 0, r = 0, minVix = null, maxVix = null, start = 0,
} = {}) {
  const trades = []
