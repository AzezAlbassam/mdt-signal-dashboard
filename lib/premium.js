 ...
 *
 *  - Skew. A one-sigma index put really trades above
 * the at-the-money volatility and the call below it. Pricing both legs flat is
 * therefore only cautious for a STRUCTURE THAT SELLS BOTH SIDES: the strangle
 * and the condor lose more on the put leg than they gain on the call, so flat
 * pricing understates their credit.
 *
 * For a one-sided structure the sign flips and it matters. Selling a call
 * alone, flat pricing OVERSTATES the credit twice over — the call trades below
 * the at-the-money vol, and VIX sits above it — so a call spread measured at
 * skew = 0 is being flattered, not stress-tested. Selling a put alone, flat
 * pricing understates it. Anything one-sided must be reported with `skew` and
 * `pricingScale` moved, not at the defaults.
