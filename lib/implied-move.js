...
/**
 * A wider band about the same centre: the inner half width times a multiple.
 * Used as the outer target once the inner band has been closed through.
 */
export function outerBand (inner, multiple) {
  if (!(multiple > 1)) throw new RangeError(`multiple must exceed 1, got ${multiple}`)
  const h = cents(inner.halfWidth * multiple)
  const c = cents(inner.center)
  return { center: c, halfWidth: h, upper: cents(c + h), lower: cents(c - h), multiple }
}

/**
 * Whether a session opened beyond a band level. A level the session opened
 * past was never available as a fill, which matters for any trade entered
 * at the band itself.
 */
export function gapThrough (band, session) {
  return { lower: session.open <= band.lower, upper: session.open >= band.upper }
}

/**
 * The same question asked of a zone rather than a single band. A horizon plots
 * two lines a side, and a session can open past one of them and not the other,
 * so the answer is per line: `near` is the line closer to the centre, `far` the
 * outer one. A side whose near line gapped but whose far line did not still
 * offers a fill, at the far line only.
 */
export function gapThroughZone (zones, session) {
  const { open } = session
  return {
    lower: { near: open <= zones.lower.high, far: open <= zones.lower.low },
    upper: { near: open >= zones.upper.low, far: open >= zones.upper.high },
  }
}

/**
 * The volatility-regime multiplier, the one band change a walk-forward test
 * supported.
 *
 * A single constant near 0.92 is too wide when the market is calm and too
 * narrow when it is not: over the decade the fixed band realised a mean |z|
 * of 0.64 in the calmest third of sessions and 0.90 in the most stressed,
 * against a true one sigma's 0.798. Refitting a multiplier per VIX tercile on
 * a rolling window, so no session is priced by its own future, cuts the
 * weighted calibration error from 0.077 to 0.011.
 *
 * It is not the default and must not become one. The published charts plot a
 * single constant, and reproducing them is what this repository is for. This
 * is the band you would draw if you wanted a true one-standard-deviation
 * range instead.
 *
 * Fitted on SPY sessions 2016-01-04 to 2026-09-14, 500-session rolling window.
 */
export const REGIME_FIT = {
  window: 500,
  fittedOn: 'SPY 2016-01-04..2026-09-14',
  fittedTo: '2026-09-12',
  baseMultiple: 0.92,
  cuts: { low: 16.35, high: 18.67 },
  k: { low: 0.7291, mid: 0.8115, high: 0.9804 },
}

/** The multiplier a volatility level earns. A cut belongs to the regime below it. */
export function regimeMultiplier (vix, fit = REGIME_FIT) {
  return vix <= fit.cuts.low ? fit.k.low : vix <= fit.cuts.high ? fit.k.mid : fit.k.high
}

/**
 * A half width rescaled from the constant multiple already inside it to the
 * one its regime earns. A band drawn at 0.92 × VIX becomes a band drawn at
 * k(regime) × VIX without needing the volatility itself.
 */
export function calibratedHalfWidth ({ halfWidth, vix, fit = REGIME_FIT, round = true }) {
  const base = fit.baseMultiple
  if (!(base > 0)) throw new RangeError(`baseMultiple must be positive, got ${base}`)
  const scaled = halfWidth * (regimeMultiplier(vix, fit) / base)
  return round ? cents(scaled) : scaled
}
