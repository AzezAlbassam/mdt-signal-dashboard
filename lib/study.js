...
/** Fraction of a calendar day a mid-session entry leaves before the close. */
const HALF_SESSION_DAYS = 0.5 * (6.5 / 24)
 ...
 * bell, with half a session assumed to remain at entry. dte 1 and above are
 * entered at that session's close and closed at the next session's close.
 */
export function simulateSetup (sessions, { setup, dte, k = DEFAULT_K, ivScale = DEFAULT_K, slippage = 0, r = 0 }) {
  const series = dailyBandSeries(sessions, { k })
  const index = new Map(sessions.map((s, i) => [s.date, i]))
  ...
  for (const b of series) {
    ...
    if (dte === 0) {
      ...
      const leg = optionLegPnl({
        ...
        daysToExpiryAtEntry: HALF_SESSION_DAYS, daysHeld: HALF_SESSION_DAYS,
        sigmaEntry: sigma, sigmaExit: sigma, r, slippage,
      })
      trades.push({
        setup, dte, type, strike: K, entryDate: s.date, exitDate: s.date, expiryDate: s.date,
        entryLevel: entryS, entryS, exitS: s.close,
        daysToExpiryAtEntry: HALF_SESSION_DAYS, daysHeld: HALF_SESSION_DAYS, ...leg,
      })
      continue
    }
