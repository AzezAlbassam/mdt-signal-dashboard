...
export function optionLegPnl ({
  S0, S1, K, type, daysToExpiryAtEntry, daysHeld, sigmaEntry, sigmaExit, r = 0,
  slippage = 0, halfSpread = 0, commission = 0,
}) {
  ...
  // A commission is charged per contract and a contract is a hundred shares.
  const ticket = halfSpread + commission / 100
  const entryCost = slippage * premiumIn + ticket
  // Nothing is paid to get out of a leg that expired worthless.
  const exitCost = premiumOut > 0 ? slippage * premiumOut + ticket : 0
  const costs = entryCost + exitCost
  const pnl = (premiumOut - premiumIn) - costs
  return { premiumIn, premiumOut, costs, pnl, pnlPct: premiumIn > 0 ? pnl / premiumIn : 0 }
}
 ...
 */
export function simulateSetup (sessions, {
  setup, dte, k = DEFAULT_K, ivScale = DEFAULT_K, slippage = 0, r = 0,
  entryFraction = 0.5, excludeGapThrough = false, volAnchor = 'knownAtEntry',
  halfSpread = 0, commission = 0,
}) {
  if (volAnchor !== 'knownAtEntry' && volAnchor !== 'anchor') throw new RangeError(`unknown volAnchor ${volAnchor}`)
  const series = dailyBandSeries(sessions, { k })
  const index = new Map(sessions.map((s, i) => [s.date, i]))
  ...
  for (const b of series) {
    ...
    if (dte === 0) {
      ...
      // A same-session leg is entered intraday, so the last VIX close known
      // when the order is sent is the anchor's, whichever convention is asked for.
      ...
      const leg = optionLegPnl({
        ...
        sigmaEntry: sigma, sigmaExit: sigma, r, slippage, halfSpread, commission,
      })
      trades.push({
        ...
        daysToExpiryAtEntry: remaining, daysHeld: remaining,
        sigmaEntry: sigma, sigmaExit: sigma, volAnchor, ...leg,
      })
      continue
    }
    ...
    // A held leg is entered at the close, when that session's VIX close is
    // known. Pricing it on the anchor instead is the only way to read the
    // ladder as one experiment, because the same-session rung has no choice.
    const sigmaEntry = ivScale * (volAnchor === 'anchor' ? prev.vix : s.vix) / 100
    const sigmaExit = ivScale * nxt.vix / 100
    const leg = optionLegPnl({
      S0: s.close, S1: nxt.close, K, type, daysToExpiryAtEntry, daysHeld,
      sigmaEntry, sigmaExit, r, slippage, halfSpread, commission,
    })
    trades.push({
      ...
      daysToExpiryAtEntry, daysHeld, sigmaEntry, sigmaExit, volAnchor, ...leg,
    })
  }
  return trades
}
...
export function summariseTrades (trades) {
  ...
  if (n === 0) {
    return { n: 0, winRate: wilsonRate(0, 0), meanPnlPct: null, medianPnlPct: null, meanPnl: null, profitFactor: null, worstPct: null, bestPct: null, meanPremiumIn: null, meanCostShare: null, costShareAggregate: null }
  }
  ...
  return {
    ...
    // The mean of the per-trade shares and the share of the total are different
    // numbers here, and the gap is the point: a fixed ticket cost is a far larger
    // share of a cheap option, and the cheap ones are most of the sample.
    meanCostShare: trades.reduce((a, t) => a + (t.premiumIn > 0 ? (t.costs ?? 0) / t.premiumIn : 0), 0) / n,
    costShareAggregate: trades.reduce((a, t) => a + (t.costs ?? 0), 0) / trades.reduce((a, t) => a + t.premiumIn, 0),
  }
}
...
export function regimeAdjustedSeries (sessions, { cuts, k }) {
  ...
  for (let i = 1; i < sessions.length; i++) {
    ...
    out.push({ anchorDate: prev.date, anchorVix: prev.vix, session: sessions[i], center: prev.close, halfWidth, upper: prev.close + halfWidth, lower: prev.close - halfWidth, regime, k: k[regime] })
  }
  return out
}
...
// ─────────────────────────────────────────────── scoring calibration honestly

/** Which VIX regime a level sits in. A cut belongs to the regime below it. */
export function regimeOf (vix, cuts) {
  return vix <= cuts.low ? 'low' : vix <= cuts.high ? 'mid' : 'high'
}

/**
 * Calibration error scored inside each regime as well as pooled.
 *
 * A pooled mean |z| is satisfied by any set of multipliers whose errors
 * happen to cancel: a band far too wide in calm markets and too narrow in
 * stressed ones scores beautifully overall and is wrong every single day.
 * The weighted figure is the one that decides anything.
 */
export function regimeError (series, cuts) {
  const groups = { low: [], mid: [], high: [] }
  for (const b of series) groups[regimeOf(b.anchorVix, cuts)].push(b)
  const byRegime = {}
  let weighted = 0
  for (const g of ['low', 'mid', 'high']) {
    const rows = groups[g]
    if (rows.length === 0) { byRegime[g] = { n: 0, meanAbsZ: null, error: null }; continue }
    const c = calibrateSeries(rows)
    const error = Math.abs(c.meanAbsZ - THEORY.meanAbsZ)
    byRegime[g] = { n: rows.length, meanAbsZ: c.meanAbsZ, error }
    weighted += rows.length * error
  }
  const all = calibrateSeries(series)
  return {
    n: series.length,
    cuts,
    pooled: Math.abs(all.meanAbsZ - THEORY.meanAbsZ),
    weighted: weighted / series.length,
    meanAbsZ: all.meanAbsZ,
    byRegime,
  }
}

/**
 * The regime multiplier refitted on a rolling window, so every session is
 * scored by a fit that ended before it opened. This is the honest test the
 * single fixed split could not give: a split fits once and then drifts, and
 * a pooled score lets its errors cancel.
 */
export function rollingRegime (sessions, { window = 250, k = DEFAULT_K, keepSeries = false } = {}) {
  const unit = dailyBandSeries(sessions, { k: 1 })
  const absZ1 = unit.map((b) => Math.abs((b.session.close - b.center) / b.halfWidth))
  const fixed = []
  const regime = []
  const rows = []
  let last = null
  for (let i = window; i < unit.length; i++) {
    const fitFrom = i - window
    const vix = []
    for (let j = fitFrom; j < i; j++) vix.push(unit[j].anchorVix)
    vix.sort((a, b) => a - b)
    const at = (q) => vix[Math.min(vix.length - 1, Math.floor(q * vix.length))]
    const cuts = { low: at(1 / 3), high: at(2 / 3) }
    const sum = { low: 0, mid: 0, high: 0 }
    const cnt = { low: 0, mid: 0, high: 0 }
    for (let j = fitFrom; j < i; j++) {
      const g = regimeOf(unit[j].anchorVix, cuts)
      sum[g] += absZ1[j]; cnt[g]++
    }
    const fitK = {}
    for (const g of ['low', 'mid', 'high']) fitK[g] = cnt[g] > 0 ? (sum[g] / cnt[g]) / THEORY.meanAbsZ : k
    const b = unit[i]
    const g = regimeOf(b.anchorVix, cuts)
    const scale = (mult) => ({
      anchorDate: b.anchorDate,
      anchorVix: b.anchorVix,
      session: b.session,
      center: b.center,
      halfWidth: b.halfWidth * mult,
      upper: b.center + b.halfWidth * mult,
      lower: b.center - b.halfWidth * mult,
    })
    fixed.push(scale(k))
    regime.push(scale(fitK[g]))
    if (keepSeries) rows.push({ date: b.session.date, fitTo: unit[i - 1].session.date, regime: g, k: fitK[g], cuts })
    last = { cuts, k: fitK }
  }
  const wholeCuts = vixTerciles(sessions)
  const out = {
    window,
    n: fixed.length,
    cuts: last.cuts,
    k: last.k,
    fixed: regimeError(fixed, wholeCuts),
    regime: regimeError(regime, wholeCuts),
  }
  if (keepSeries) out.series = rows
  return out
}
...
/**
 * What happens to the outer band once the inner one has been closed through,
 * split into the part that was already over by the time the signal existed
 * and the part that was still ahead. Only the second was ever a trade.
 */
export function outerAfterBreak (sessions, { k = DEFAULT_K, multiple = 2 } = {}) {
  const series = dailyBandSeries(sessions, { k })
  const index = new Map(sessions.map((s, i) => [s.date, i]))
  let n = 0; let same = 0; let nextOnly = 0; let skipped = 0
  for (const b of series) {
    const s = b.session
    const up = s.close > b.upper
    const down = s.close < b.lower
    if (!up && !down) continue
    const nxt = sessions[index.get(s.date) + 1]
    if (!nxt) { skipped++; continue }
    n++
    const level = up ? b.center + multiple * b.halfWidth : b.center - multiple * b.halfWidth
    const hit = (row) => (up ? row.high >= level : row.low <= level)
    if (hit(s)) same++
    else if (hit(nxt)) nextOnly++
  }
  return {
    n,
    multiple,
    skipped,
    reached: wilsonRate(same + nextOnly, n),
    sameSession: wilsonRate(same, n),
    nextSessionOnly: wilsonRate(nextOnly, n),
    tradeable: wilsonRate(nextOnly, n - same),
  }
}

/** A seeded generator, so a simulated base rate is a fixture and not a coin toss. */
...
/**
 * How often a driftless random walk that closed beyond ±halfWidth reaches
 * ±multiple × halfWidth during that session or the next. Widths are in units
 * of the realised daily standard deviation, so halfWidth 1.036 is a band
 * calibrated the way the measured one is.
 */
export function reachAfterBreakBaseRate ({
  paths = 60000, steps = 78, halfWidth = 1, multiple = 2, seed = 1,
} = {}) {
  ...
  const step = 1 / Math.sqrt(steps)
  const outer = multiple * halfWidth
  let kept = 0; let hits = 0
  for (let p = 0; p < paths; p++) {
    let w = 0; let mx = 0; let mn = 0
    for (let i = 0; i < steps; i++) { w += gauss() * step; if (w > mx) mx = w; if (w < mn) mn = w }
    if (Math.abs(w) <= halfWidth) continue
    kept++
    const up = w > halfWidth
    let reached = up ? mx >= outer : mn <= -outer
    if (!reached) {
      let w2 = w; let mx2 = w; let mn2 = w
      for (let i = 0; i < steps; i++) { w2 += gauss() * step; if (w2 > mx2) mx2 = w2; if (w2 < mn2) mn2 = w2 }
      reached = up ? mx2 >= outer : mn2 <= -outer
    }
    if (reached) hits++
  }
  return { ...wilsonRate(hits, kept), paths, steps, halfWidth, multiple, seed }
}
