...
export function sigmaAt (chain, asOf, days, curve = termStructure(chain, asOf)) {
  return sigmaFromCurve(curve, days)
}

/**
 * The same reading, from any curve of {days, iv} points rather than a chain.
 * Interpolated in total variance because that is what is additive in time,
 * and clamped outside the published range so a short curve cannot invent a
 * number it was never told.
 */
export function sigmaFromCurve (curve, days) {
  if (!curve || curve.length === 0) throw new Error('term structure is empty; cannot read a volatility')
...
/**
 * The tenors the exchange publishes a volatility index for, in calendar days.
 * All four are free and daily, which makes them the cheapest real improvement
 * available to this indicator: a daily band wants the nine-day point and has
 * been reading the thirty-day one.
 */
export const VIX_TENORS = { vix9d: 9, vix: 30, vix3m: 93, vix6m: 186 }

/** A curve from whichever volatility indices are to hand, quoted in per cent. */
export function vixCurve (quotes = {}) {
  return Object.entries(VIX_TENORS)
    .filter(([key]) => Number.isFinite(quotes[key]) && quotes[key] > 0)
    .map(([key, days]) => ({ days, iv: quotes[key] / 100, source: key }))
